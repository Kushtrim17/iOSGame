...
- (void)moveTowardsPosition:(CGPoint)position
{
    ...
    //float slope = (self.position.y - position.y) / (self.position.x - position.x);
    //NSLog(@"This is the slope %f",slope);
    
    //slope = (y2-y1)/(x2-x1) - we dont know y2
    //y2-y1 = slope * (x2-x1)
    //y2 = slope(x2-x1) + y1
    float offscreenX;
    
    if (position.x <= self.position.x) {
        offscreenX = -10; //its on the left side
    }
    else {
    ...
    //NSLog(@"offscreenX is :: %f",offscreenX);
    //NSLog(@"offscreenY is :: %f",offscreenY);
